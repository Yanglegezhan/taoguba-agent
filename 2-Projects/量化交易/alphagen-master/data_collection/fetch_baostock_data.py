...
class DataManager:
    ...
    def _download_stock_data_job(self, code: str, data: pd.Series) -> None:
        ...
        def calculate_ad(close, low, high, volume):
            clv = ((close - low) - (high - close)) / (high - low)
            clv.fillna(0, inplace=True)  # 处理除以0的情况
            ad = clv * volume
            ad = ad.cumsum()
            return ad

        def calculate_adosc(ad, fast_period=3, slow_period=10):
            ad_ema_fast = ad.ewm(span=fast_period).mean()
            ad_ema_slow = ad.ewm(span=slow_period).mean()
            adosc = ad_ema_fast - ad_ema_slow
            return adosc

        def calculate_adx(high, low, close, n=14):
            plus_di = 100 * pd.Series(
                np.where((high - high.shift(1)) > (low.shift(1) - low), high - high.shift(1), 0)).rolling(
                window=n).mean() / high
            minus_di = 100 * pd.Series(
                np.where((low.shift(1) - low) > (high - high.shift(1)), low.shift(1) - low, 0)).rolling(
                window=n).mean() / low
        ...
        df["ad"] = calculate_ad(df['close'], df['low'], df['high'], df['volume'])
        df["adosc"] = calculate_adosc(df['ad'])
