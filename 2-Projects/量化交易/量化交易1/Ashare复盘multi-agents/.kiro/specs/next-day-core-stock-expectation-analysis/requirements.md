...
- **Recognition_Stock**: 辨识度个股，具有明显技术特征的个股
...
3. THE System SHALL 在 AKShare_API 调用前自动配置代理设置
...
3. THE Stage1_Agent SHALL 识别处于技术支撑位的 Recognition_Stock
4. THE Stage1_Agent SHALL 识别走趋势的核心个股（近期沿均线上涨的个股）
5. THE Stage1_Agent SHALL 记录每只个股的连板高度、板块归属、市值、换手率等基础信息
6. THE Stage1_Agent SHALL 将识别的个股添加到 Gene_Pool 并更新其历史记录
7. THE Stage1_Agent SHALL 为每只个股生成唯一标识符用于跨日追踪
