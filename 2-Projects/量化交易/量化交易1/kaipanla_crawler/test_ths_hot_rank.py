...
def test_ths_hot_rank():
    print('=' * 60)
    print('Start testing THS hot rank data')
    print('=' * 60)
    crawler = KaipanlaCrawler()
    print('Fetching THS hot rank 30 stocks...')
    print('Note: First run may need selenium and chromedriver')
    series = crawler.get_ths_hot_rank(headless=False, wait_time=5)
    if series.empty:
        print('Failed: Empty data returned')
        return False
    print('Success!')
    print(f'Data count: {len(series)}')
